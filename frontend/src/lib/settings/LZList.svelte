...
<div>
  ...
  <div>
    {#each $allLandingZones as lz (lz.homePageId)}
      <div
        class="my-2 py-2 px-4 bg-surface-200-700-token rounded-md border-2 border-transparent flex hover:bg-surface-100-800-token hover:border-2 hover:text-xl transition-all hover:border-surface-200-700-token"
      >
        ...
        <div class="flex justify-end flex-1 space-x-2">
          {#if $primaryLandingZone?.homePageId !== lz.homePageId}
            <button
              ...
              class="btn variant-filled-warning btn-sm transition-all"
            ...
            <button
              transition:fly|fade={{ x: -50, delay: 10 }}
              class="btn variant-filled-primary btn-sm transition-all"
              class:opacity-50={$isLandingZoneLoading || $isSetPrimaryLZLoading}
              disabled={$isLandingZoneLoading || $isSetPrimaryLZLoading}
              type="button"
